...
export default function ManageStudentsTab({ actor }: ManageStudentsTabProps) {
  ...
  const fetchStudents = useCallback(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      setStudents(users.sort((a,b) => (a.displayName || "").localeCompare(b.displayName || "")));
    }
    setIsLoading(false);
  }, [actor]);
              ...
              <TableBody>
                {filteredStudents.map(student => (
                  <TableRow key={student.uid}>
                    ...
                    <TableCell>
                      {student.isApproved ? <Badge variant="default" className="bg-green-500 hover:bg-green-600">Approved</Badge> : 
